...
impl AppsManagerClient {
    ...
    fn create_app_content_tool() -> rmcp::model::Tool {
        rmcp::model::Tool::new(
            ...
            "Generate content for a new CodeMindX app. Returns the HTML code, app name, description, and window properties.".to_string(),
    ...
    fn update_app_content_tool() -> rmcp::model::Tool {
        rmcp::model::Tool::new(
            ...
            "Generate updated content for an existing CodeMindX app. Returns the improved HTML code, updated description, and optionally updated window properties.".to_string(),
...
impl McpClientTrait for AppsManagerClient {
    async fn list_tools(
        ...
        let tools = vec![
            McpTool::new(
                "list_apps".to_string(),
                "List all available CodeMindX apps with their names and descriptions. Use this to see what apps exist before creating or modifying apps.".to_string(),
                schema::<ListAppsParams>(),
            ),
            McpTool::new(
                "create_app".to_string(),
                "Create a new CodeMindX app based on a description or PRD. The extension will use an LLM to generate the HTML/CSS/JavaScript. Apps are sandboxed and run in standalone windows.".to_string(),
